// src/renderer/screens/mainScreens/screen/controller/ForestTempleScreenController.ts
...
export class ForestTempleScreenController implements MainScreenController {
    // ====================
    // DOM / 描画関連
    // ====================
    ...
    private lastPlayerPos: WorldPxPosition = { x: 0, y: 0 };
    ...
    init(root: HTMLElement, initCtx: ScreenInitContext) {
        ...
        this.screen.id = "forestTempleScreen";
    ...
    update(delta: number, frame: InputFrame) {
        ...
        const result = this.playerController.update(this.lastPlayerPos, frame.gameAxisIntents, delta);
        ...
        // ワールド描画
        this.worldRenderer.render(this.world, this.objectLayer, camera, playerPos, this.lastPlayerState, this.zones,
            this.npcs, this.signs, this.items);
        ...
        for (const e of events) {
            switch (e.action) {
                ...
                case "TEST_CHANGE_WORLD":
                    this.emitWorld?.({ type: "CHANGE_WORLD", mapId: MapId.WORLD_MAP });
                    break;
