// src/renderer/screens/mainScreens/screen/controller/GraveCaveScreenController.ts
...
export class GraveCaveScreenController implements MainScreenController {
    // ====================
    // DOM / 描画関連
    // ====================
    ...
    update(delta: number, frame: InputFrame) {
        ...
        // 現在のプレイヤー座標
        const pos = this.gameState.where.GRAVE_CAVE;
            ...
            switch (e.action) {
                case "CONFIRM":
                    this.emitUI?.({
                        ...
                        playerPosition: this.lastPlayerPos,
                        npcs: this.npcs,
                        signs: this.signs,
                        items: this.items
                    });
                ...
                case "TEST_CHANGE_WORLD":
                    this.emitWorld?.({ type: "CHANGE_WORLD", mapId: MapId.FOREST_TEMPLE });
                    break;
    ...
    setWorld(def: WorldDefinition) {
        ...
        this.lastPlayerPos = { ...this.gameState.where.GRAVE_CAVE };
    }
}
