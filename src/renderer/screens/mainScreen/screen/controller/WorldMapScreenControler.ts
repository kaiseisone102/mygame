...
import { UseGameInputScreenController } from "../../../../../renderer/screens/interface/controller/UseGameInputScreenController";
...
export class WorldMapScreenController implements MainScreenController, UseGameInputScreenController {
    // ====================
    // DOM / 描画関連
    // ====================
    ...
    update(delta: number, frame: InputFrame) {
        ...
        // プレイヤー座標
        const pos = this.gameState.where.WORLD_MAP;
        ...
        // ワールド描画
        this.worldRenderer.render(this.world, this.objectLayer, camera, pos,
            this.lastPlayerState, this.zonePx, this.npcs, this.signs, this.items);
    ...
    // screenManager からの 軸入力は InputState に流す
        ...
        for (const e of events) {
            switch (e.action) {
                ...
                case "TEST_CHANGE_WORLD":
                    this.emitWorld?.({ type: "CHANGE_WORLD", mapId: MapId.NO_FEATURE_TOWN });
                    break;
    ...
    setWorld(def: WorldDefinition) {
        ...
        this.lastPlayerPos = { ...this.gameState.where.WORLD_MAP };
    }
}
