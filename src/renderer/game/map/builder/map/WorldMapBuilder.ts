...
export class WorldMapBuilder implements MapBuilder {
    ...
    build(): BaseWorldDefinition {
        ...
        fillRectTile(world, 0, 199, 0, 199, TileType.PLAIN);
        ...
        if (this.buildingSquare) {
            ...
            const poorHouse = this.buildingSquare.buildings.find(
                b => b.id === BuildingSquareId.BUILDING_POOR_SQUARE_01
            );

            if (poorHouse) {
                placeBuildingSquare(world, 20, 180, poorHouse);
            }
        }
